...
def build_graph(MCd: dict, HCd: dict):
    ...
    with g.as_default():
        ...
        # that could help replace this.
        # layer_names = create_layer_names(list(HCd["layers"]))
        layer_names = [b.name.replace("/", "__").replace(":", "_") for b in bias]

        assert len(bias) == len(
            layer_names
        ), "number of bias params not equal to number of layers"
