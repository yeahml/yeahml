...
def build_model(main_cdict: dict, model_cdict: dict):
    # logger = logging.getLogger("build_logger")
    logger = config_logger(main_cdict, "build")
    logger.info("-> START building graph")

    try:
        reset_graph_deterministic(main_cdict["seed"])
    except KeyError:
        reset_graph()

    g_logger = config_logger(main_cdict, "graph")
    ...
    # created.
    if main_cdict["input_layer_dim"][0] == -1:
        input_layer = tf.keras.Input(shape=(main_cdict["input_layer_dim"][1:]))
    else:
        input_layer = tf.keras.Input(shape=(main_cdict["input_layer_dim"]))
